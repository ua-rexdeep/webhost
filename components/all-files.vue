<template>
    ...
    <div class="grid grid-cols-2 gap-2 overflow-y-scroll overflow-x-hidden pr-1" id="all-files-list">
        ...
        <div class="flex flex-col w-full justify-between group bg-[#333539] px-2 rounded-md cursor-pointer py-2 relative"
            v-for="file in filesWithSortedBy" @click="CopyIDToClipboard(file.id + '.' + file.type)">
...
function CopyIDToClipboard(text) {
    ...
    navigator.clipboard.writeText(`http://${cfg.public.SERVER_IP}:3092/api/file/${text}`);
}
