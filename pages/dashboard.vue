<template>
    ...
    <div class="w-full h-full flex" v-if="!fileStore.roles.includes('peof2')">
        ...
        <div class="w-full flex p-4 bg-[#222326] m-4 ml-2 rounded-xl" :class="{ 'items-center justify-center': !uploadFile }">
            <div class="flex gap-2 items-center text-xl cursor-pointer p-10" @click="uploadFileRef.click()" v-if="!uploadFile">
            ...
            <div v-else class="flex flex-col gap-3 w-full">
                ...
                <div class="flex gap-3 items-center">
                    <input class="outline-none px-3 py-1.5 w-56 rounded-md border border-[#333539] text-white placeholder:text-white bg-[#191a1c]" placeholder="File name" 
                        v-model="uploadFileName" />
                    <div class="flex gap-2 items-center"><i class="fa-solid fa-triangle-exclamation text-[#C8A345]"></i> Тільки англійські букви, прочерк та підкреслення</div>
                </div>
                <div>
                    <button class="py-1.5 px-4 rounded-md bg-[#333539] cursor-no-drop w-56" :class="{ 'bg-success cursor-pointer': canUpload }"
